<script lang="ts">
    ...
    let products1Months: Product[] = []; // กำหนดประเภทเป็น array เพื่อเก็บสินค้า 1 เดือน
    ...
    async function fetchProducts() {
      try {
        const res1Months = await fetch('http://localhost:3000/products/1months');
    ...
    async function buyProduct(productID) {
      ...
      if (balance < totalPrice) {
        alert('ยอดเงินของคุณไม่เพียงพอสำหรับการสั่งซื้อ!');
        return;
      }

      try {
        const response = await fetch(`http://localhost:3000/products/1months/buy/${userID}/${productID}`, {
          method: 'POST',
          headers: {
        ...
        {#if products1Months.length > 0}
          <div>
            <button class="flex flex-col items-center justify-center bg-[#FFE3DE] rounded-3xl shadow-md p-10 w-[500px] h-[600px] mr-10 mb-10">
              ...
              <h1 class="text-4xl font-semibold text-[#2C2C2C] ">IN STOCK {products1Months.length}</h1>
            </button>
          </div>
